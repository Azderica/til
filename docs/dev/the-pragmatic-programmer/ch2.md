...
> Q. C++는 다중 상속을 지원하고, 자바는 다중 인터페이스를 구현 / 다중 상속과 다중 인테페이스 사용이 직교성에 끼치는 영향이 다른지, 위임과 상속의 차이
...
- 프로토타입은 제한된 몇 가지 질문에 답할 목적으로 설계되기 때문에 실제 젶무보다 훨씬 적은 비용으로 빠르게 개발할 수 있습니다.
...
- 대부분의 애플리케이션이 예상 수명보다 더 오래가기 때문에 가독성 좋은 언어를 채ㅔ택하는 편이 더 좋습니다.
